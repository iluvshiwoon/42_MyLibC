...
/*   Updated: 2024/01/30 18:48:11 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void *my_realloc(void * p_origin, size_t origin_size, size_t requested_size)
{
    ...
    else if (requested_size < origin_size)
    {
        ...
        if (r_pointer)
            return(ft_memcpy(r_pointer, (const void *)p_origin, requested_size), free (p_origin), r_pointer);
    }
    else if (requested_size > origin_size)
    {
        r_pointer = malloc(requested_size);    
        if (r_pointer)
            return(ft_memcpy(r_pointer, (const void *)p_origin, origin_size), free (p_origin), r_pointer);
    }
    else if (requested_size == origin_size)
        return (p_origin);
    return (NULL);
